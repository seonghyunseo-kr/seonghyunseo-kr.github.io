---
layout: post
type: publication
year: 2026
title: "SHIFT: Self-Healing Intelligence in Feature Store and Data Store Transitions"
authors: "Seo, S.-H.; Lim, D.-J."
venue: "IEEE Access (Volume: 14)"
paper_url: "https://ieeexplore.ieee.org/abstract/document/11366656"
description: "First Author Research Paper"
# tags: [virtual-metrology, manufacturing, mlops]
style: border
color: primary
comments: false
toc: true
---

<br>


> **Journal**: IEEE Access (Volume: 14)  
> **DOI**: [10.1109/ACCESS.2026.3658719](https://ieeexplore.ieee.org/abstract/document/11366656)  
> **Date of Publication**: 28 January 2026  
> **eISSN**: 2169-3536

<br>

### Abstract
This study presents an AI model management framework for maintaining the prediction performance of virtual metrology (VM) systems in steel manufacturing processes. The proposed method addresses concept drift (CD), a major threat to long-term model reliability, by employing a dual-update strategy that autonomously updates both the feature store and the data store. Unlike conventional model maintenance approaches based on static inputs and accumulated data, our framework continuously reassesses feature relevance and adapts retraining datasets to reflect evolving process conditions. In a real-world steel sintering process, the method achieved significant recovery in predictive accuracy—from 65% post-deployment to over 93% after updates. This study highlights the importance of proactive maintenance for AI models and demonstrates how an intelligent update strategy can extend the usable life and reliability of VM systems in critical production workflows. The findings contribute to the broader field of AI-enabled asset management by offering a scalable solution for condition monitoring, retraining scheduling, and long-term performance assurance.

**Key Words: Adaptation models, Data models, Artificial intelligence, Reliability, Maintenance, Predictive models, Monitoring, Concept drift, Feature extraction, Steel Manufacturing**

### Overview
![SHIFT Overview](/assets/img/SHIFT_Overview.jpg)

*Overview of the proposed method: SHIFT (Left) and comparison of predictive performance (right).*


<br>

#### Contribution
- SeongHyun Seo: First Author
- DongJoon Lim: Corresponding Author & Advisor 

#### Notes
This journey began during my undergraduate years through an industry-academic collaboration and was finalized during my Master’s degree. It is incredibly rewarding to see this long-term effort come to fruition.